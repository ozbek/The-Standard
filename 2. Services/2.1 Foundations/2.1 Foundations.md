...
## 2.1.4 Self-Sufficient (Dead-End) Services

### 2.1.4.0 Introduction

A self-sufficient service, also referred to as a dead-end service, is a service whose execution flow does not continue into any external dependency.

These services:

- do not call brokers
- do not call other services
- do not orchestrate external workflows
- contain only internal business or computational logic

The flow effectively “ends” within the service itself, which is why the term dead-end service is sometimes used.

### 2.1.4.1 Characteristics

A self-sufficient service typically:

- operates entirely in-memory
- produces deterministic outcomes
- relies only on the provided input parameters
- contains pure business rules or calculations
- has no dependency injection requirements

### 2.1.4.2 Design Guidance

Self-sufficient services should remain:

- focused
- deterministic
- side-effect free where possible
- easy to test without infrastructure setup

Because these services do not depend on external components, they are often simpler to:
...
### 2.1.4.3 Testing Considerations

While it is generally easier to test self-sufficient services due to their lack of dependencies, 
there are some unique considerations to keep in mind for exception handling and edge cases.

Logic and validation paths are straightforward to test with various input scenarios, 
but simulating internal failures or forcing specific execution branches can be more 
challenging without external dependencies to mock.

In these situations, there may be no injectable seam available for introducing mocked behaviour.  
To address this, partial mocking techniques may be used to:
- simulate internal failures
- force deterministic execution branches
- validate internal exception handling behaviour
- isolate difficult-to-reach execution paths

This allows self-sufficient services to remain fully encapsulated while still enabling 
comprehensive unit test coverage.

#### Example

The service below is a simple example of a self-sufficient service that calculates 
the average score for a student based on an array of scores. It includes validation 
to ensure the scores are valid and demonstrates how to handle exceptions 
without relying on any external dependencies.

##### StudentScoreService.cs

```csharp
public partial class StudentScoreService
{
	public ValueTask<double> CalculateAverageScoreAsync(params int[] scores) =>
	TryCatch(async () =>
	{
	...
	internal virtual void ValidateScores(int[] scores)
	{
		if (scores is null || scores.Length == 0)
		{
			throw new InvalidStudentScoresException(
				"Student scores are invalid.");
		}
...
```csharp
[Fact]
public async Task ShouldCalculateAverageScoreAsync()
{
	// given
	int[] scores = [80, 90, 100];

	double expectedAverage = 90;

	var studentScoreService = new StudentScoreService();

	// when
	double actualAverage =
		await studentScoreService.CalculateAverageScoreAsync(scores);

	// then
	actualAverage.Should().Be(expectedAverage);
}
```
(Since there are no dependencies, we do not see any of the usual Setup, Verify or VerifyNoOtherCalls lines)

##### Validation Test

```csharp
[Fact]
public async Task ShouldThrowValidationExceptionOnCalculateAverageScoreIfScoresAreInvalidAsync()
{
	// given
	int[] invalidScores = [];

	var invalidStudentScoresException =
		new InvalidStudentScoresException(
			"Student scores are invalid.");

	var expectedStudentValidationException =
		new StudentValidationException(
			message: "Student validation error occurred, fix errors and try again.",
			innerException: invalidStudentScoresException);

	var studentScoreService = new StudentScoreService();

	// when
	ValueTask<double> calculateAverageScoreTask =
		studentScoreService.CalculateAverageScoreAsync(invalidScores);

	// then
	StudentValidationException actualException =
		await Assert.ThrowsAsync<StudentValidationException>(
			calculateAverageScoreTask.AsTask);
...
##### Exception Test Using Partial Mocking

Since `CalculateAverageScoreAsync` does not have any dependencies, we can use partial mocking
to simulate an internal failure and validate that the exception handling logic is working as expected.

Under normal conditions we would use the first dependency executed within the method to
mock exceptions, but since there are no dependencies in this service, we will have to use
partial mocking of the service itself to throw an exception from the first method called.
In this case it is the `ValidateScores` method, which is called for validation inside
the `TryCatch` block.

Because `ValidateScores` is already declared as `internal virtual` in the service definition above,
Moq can override it on a partial mock of `StudentScoreService`.

Since `ValidateScores` is `internal`, it must be made visible to the test project.

Inside the project file of `University.ClassLibrary`, 
we can add the following lines to make the internal members visible to the test project:

```plaintext
	<ItemGroup>
		<InternalsVisibleTo Include="University.ClassLibrary.Tests.Unit" />
		<InternalsVisibleTo Include="DynamicProxyGenAssembly2" />
	</ItemGroup>
```
(DynamicProxyGenAssembly2 is required for Moq to work with internal members)

Now we can write our test using Moq to create a partial mock of the `StudentScoreService` and set up the `ValidateScores` method to throw an exception.

```csharp
[Fact]
public async Task ShouldThrowServiceExceptionOnCalculateAverageScoreIfCalculationFailsAsync()
{
	// given
	int[] scores = [80, 90, 100];
	...
	var studentScoreServiceMock =
		new Mock<StudentScoreService> { CallBase = true };

	studentScoreServiceMock
		.Setup(service => service.ValidateScores(scores))
			.Throws(failedStudentServiceException);

	// when
	ValueTask<double> calculateAverageScoreTask =
		studentScoreServiceMock.Object.CalculateAverageScoreAsync(scores);
	...
	studentScoreServiceMock
		.Verify(service => service.ValidateScores(scores),
			Times.Once);

	studentScoreServiceMock.VerifyNoOtherCalls();
}
```

The mock is created with `CallBase = true`, which instructs Moq to 
execute the real implementation of any method that has not been explicitly set up. 
This means `CalculateAverageScoreAsync` runs its actual implementation 
while `ValidateScores` is intercepted and forced to throw, 
exercising the `TryCatch` exception handling path without any external dependencies.

##### Partial Mocking When a Method Has No Arguments

When the method under test has no arguments, such as `RetrievePi` below,
there is no validation method that can be mocked directly. The same partial mocking technique still applies,
but it requires a small refactor — extract the logic inside the `TryCatch` block into a separate
`internal virtual` method so that Moq can intercept it.

Before the refactor:

```csharp
public partial class MathService : IMathService
{
	public double RetrievePi() =>
	TryCatch(() =>
	{
		return Math.PI;
	});
...
After extracting the inner logic into a mockable method:

```csharp
public partial class MathService : IMathService
{
	public virtual double RetrievePi() =>
	TryCatch(() =>
	{
		return GetPi();
	});

	internal virtual double GetPi() =>
		Math.PI;
}
```

With this refactor in place, `GetPi` can be set up to throw in the exception test, while
`RetrievePi` executes its real implementation via `CallBase = true`:

```csharp
[Fact]
public async Task ShouldThrowServiceExceptionOnRetrievePiIfServiceErrorOccursAsync()
{
	// given
	var someException =
		new Exception("Some service error.");

	var expectedMathServiceException =
		new MathServiceException(
			message: "Math service error occurred, contact support.",
			innerException: someException);

	var mathServiceMock =
		new Mock<MathService> { CallBase = true };

	mathServiceMock
		.Setup(service => service.GetPi())
			.Throws(someException);

	// when
	ValueTask<double> retrievePiTask =
		mathServiceMock.Object.RetrievePi();

	// then
	MathServiceException actualMathServiceException =
		await Assert.ThrowsAsync<MathServiceException>(
			retrievePiTask.AsTask);

	actualMathServiceException.Should().BeEquivalentTo(
		expectedMathServiceException);

	mathServiceMock
		.Verify(service => service.GetPi(),
			Times.Once);

	mathServiceMock.VerifyNoOtherCalls();
}
```